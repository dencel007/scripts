...
then
  ...
  echo -e "\033[0;91m> git branch = DTC \033[0m\n"
...
# identify os type and version - AOSP by default
export OSTYPE=AOSP
...
then
  export CLANGVERSION=$(head -n 2 "$CLANGDIR"/AndroidVersion.txt | perl -pe 's/\(http.*?\)//gs' | sed -e 's/  */ /g' -e 's/[[:space:]]*$//')
...
KERNEL_VERSION=$(head -n-1853 Makefile | tail -n +2 | sed -E 's/.*(^\w+\s[=]\s)//g' | xargs | sed -E 's/(\s)/./g')
